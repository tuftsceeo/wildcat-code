/**
 * @file SpinCounterclockwiseDash.jsx
 * @description Dashboard component for configuring motors to spin counterclockwise at selected speeds.
...
// Speed constants matching existing motor speed utilities (negative for counterclockwise)
const SPEED_VALUES = {
    slow: -330,
    medium: -660,
    fast: -1000
...
/**
 * Component for a single motor control in counterclockwise spin dashboard
 *
 * @component
...
const SingleMotorDash = memo(({ 
    ...
    const speedOptions = [
        { key: 'slow', icon: <Snail className={`${styles.speedIcon} ${styles.flippedHorizontally}`} />, label: 'Slow' },
        { key: 'medium', icon: <Turtle className={`${styles.speedIcon} ${styles.flippedHorizontally}`} />, label: 'Medium' },
        { key: 'fast', icon: <Rabbit className={`${styles.speedIcon} ${styles.flippedHorizontally}`} />, label: 'Fast' }
    ];
    ...
    return (
        ...
        >
            ...
            <div className={`${styles.motorAnimation} ${!isSelected ? styles.unknown : ''}`}>
                <div 
                    className={`${styles.motorRotor} ${isSelected && isConnected ? styles.spinningCounterclockwise : ''}`}
                    style={{
...
/**
 * Main dashboard component for configuring counterclockwise motor spinning
 *
 * @component
 * @param {Object} props - Component props
 ...
 * @returns {JSX.Element} Counterclockwise motor spin dashboard
 */
export const SpinCounterclockwiseDash = ({ 
    onUpdate, 
    configuration, 
    slotData, 
    currSlotNumber, 
    ...
    useEffect(() => {
        ...
        if (configuration) {
            ...
            configs.forEach(config => {
                if (config && config.port && config.speed < 0) { // Only negative speeds for counterclockwise
                    // Determine speed level from numeric value
                    const absSpeed = Math.abs(config.speed);
                    let speedLevel = 'fast'; // default
                    if (absSpeed <= 330) speedLevel = 'slow';
                    else if (absSpeed <= 660) speedLevel = 'medium';
    ...
    const updateConfiguration = (configs) => {
        ...
        const configArray = Array.from(configs.values()).map(config => ({
            port: config.port,
            speed: SPEED_VALUES[config.speed] // Convert to numeric value (negative)
        }));
    ...
    return (
        <div className={styles.dashboardContainer}>
            <div className={styles.dashboardTitle}>Spin Counterclockwise</div>
...
export default memo(SpinCounterclockwiseDash);
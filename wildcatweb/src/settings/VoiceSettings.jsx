...
const VoiceSettings = () => {
  ...
  const renderIcon = (iconName) => {
    switch (iconName) {
      case "Bot":
        return <Bot size={24} strokeWidth={2.75}/>;
      case "Zap":
        return <Zap size={24} strokeWidth={2.75}/>;
      case "Bird":
        return <Bird size={24} strokeWidth={2.75}/>;
      default:
        return <Bot size={24} strokeWidth={2.75}/>;
    }
  };
  ...
  useEffect(() => {
    const matchingPreset = VOICE_PRESETS.find(preset => preset.id === voice);
    if (matchingPreset) {
  ...
  return (
    <div className={styles.container}>
      <h2 className={styles.title}>{isSpanish ? "Voces de Robot" : "Robot Voices"}</h2>
      ...
      <div className={styles.previewContainer}>
        <div className={styles.previewTitle}>
          ...
          <button
            ...
            aria-label={isSpanish ? "Reproducir muestra" : "Play sample with current voice"}
          >
            <Volume2 size={20} />
      ...
      <div className={styles.voiceOptions}>
        <h3 className={styles.sectionTitle}>
          {isSpanish ? "Elige Tu Voz de Robot" : "Choose Your Robot Voice"}
        </h3>
          ...
          {VOICE_PRESETS.map((preset) => (
            ...
            >
              ...
              <div className={styles.voiceInfo}>
                <div className={styles.voiceName}>{preset.name}</div>
                <div className={styles.voiceDescription}>{
                  isSpanish ? getSpanishDescription(preset.id) : preset.description
                }</div>
              </div>
            </button>
          ))}
      ...
      <div className={styles.volumeControl}>
        <h3 className={styles.sectionTitle}>{isSpanish ? "Volumen" : "Volume"}</h3>
        ...
        <div className={styles.volumeSlider}>
          ...
          <input
            ...
            aria-label={isSpanish ? "Volumen de voz" : "Voice volume"}
          />
          <Volume2 size={20} />
